...
FileHook::FileHook(const std::string &path, const std::string &str,
...
void FileHook::ComposeString(std::size_t offset, std::string *str) {
...
auto FileHook::SelectPositionDependentAction(FileReader::OffsetPosition pos) {
...
void FileHook::ReadFirstSegment() {
	file_reader_.Read();
	itr_ = search(file_reader_.cbegin(), file_reader_.cend(), str_.begin(), str_.end());
}

//todo file_reader_ as part of this object, instead of being pointer
std::string FileHook::Search() {
	if (itr_ == nullptr) {
		ReadFirstSegment();
	}
